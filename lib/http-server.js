...
exports.createServer = function (options) {
  ...
  require('chokidar-socket-emitter')({
    app: server.server,
    path: '/Users/thaol/gitrepos/stark/web/src/main/webapp'
  });
  return server;
};
...
function HttpServer(options) {
  ...
  if (typeof options.proxy === 'string') {
    ...
    proxy.on('error', function (err, req, res) {
      console.log("error for proxy request");
    ...
    proxy.on('proxyRes', function (proxyRes, req, res) {
      //console.log('RAW Response from the target', util.inspect(proxyRes, { showHidden: true, depth: null }));
    });
